...
export function StoreProvider({
  ...
  useEffect(() => {
    ...
    fetch('/api/employees').then(r => r.ok && r.json().then(d => useHrStore.getState().setEmployees(Array.isArray(d) ? d : (d.items ?? [])))).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    const fetchLeave = () => fetch('/api/leave-requests').then(r => r.ok && r.json().then(data => {
      if (data.requests) useHrStore.getState().setLeaveRequests(data.requests)
      if (data.balances) useHrStore.getState().setLeaveBalances(data.balances)
    })).catch(() => {})
    fetchLeave()
  ...
  useEffect(() => {
    useHrStore.getState().setHrContext({
...
const storeCtx: AppState = {
   ...
   // Sales & Invoicing
    ...
    uploadMyDocument: (document) => {
      ...
      useHrStore.getState().setHRDocuments(prev => [doc, ...prev])
