...
export interface CompanySettings {
  ...
  /** Odoo Configure Document Layout values used by all commercial PDFs. */
  printTemplate?: import('@/lib/document-layout').DocumentLayoutId | 'classic' | 'light' | 'modern' | 'compact'
  printFont?: import('@/lib/document-layout').DocumentFontId
  printBackground?: import('@/lib/document-layout').DocumentBackgroundId
  printPrimaryColor?: string
  printSecondaryColor?: string
  printTagline?: string
  printPaperFormat?: import('@/lib/document-layout').DocumentPaperFormat
  /**
...
export function StoreProvider({
  ...
  const afterSalesActions = useMemo(() => ({
    ...
    creditBuyBack: (...args: Parameters<AppState['creditBuyBack']>) => storeCtxRef.current!.creditBuyBack(...args),
...
const storeCtx: AppState = {
   ...
   // Sales & Invoicing
    ...
    createTradeInFromRepair: (repairId, opts) => {
      ...
      showToast(`${repair.ref} → trade-in ${bb.ref} (draft). Approve, then pay or add as credit in Trade-in.`, 'success')
    ...
    closePOSSession: (closingCash) => {
      ...
      const tenderAmount = (order: POSOrder) => Math.max(0, order.total - (order.customerCreditAmount || 0))
      const totalCash = orders.filter(o => o.payment === 'cash').reduce((a, o) => a + tenderAmount(o), 0)
      const totalMpesa = orders.filter(o => o.payment === 'mpesa').reduce((a, o) => a + tenderAmount(o), 0)
      const totalBank = orders.filter(o => isPosBankPayment(o.payment)).reduce((a, o) => a + tenderAmount(o), 0)
      ...
      for (const o of orders) {
        ...
        bankByAccount.set(id, (bankByAccount.get(id) || 0) + tenderAmount(o))
      }
    ...
    createPOSOrder: async (lines, payment, customerId, customerName, pointsRedeemed = 0, applyVat = false, paymentMeta) => {
      ...
      if (accountingPostError) {
        ...
        showToast(
          `${order.ref} · ${fmtKes(order.total)} via ${tenderTotal > 0 ? payLabel : 'CLIENT CREDIT'}`
            + (tenderTotal > 0 && bankLabel ? ` (${bankLabel})` : '')
            + creditLabel
    ...
    payBuyBack: (id, paymentMethod) => {
      const user = currentUser(); if (!user) return
      const bb = buyBacks.find(b => b.id === id)
      const allowed = canPayBuyBackCash(bb, paymentMethod)
      if (!allowed.ok) { showToast(allowed.error || 'Cannot record buy-back payment', 'error'); return }
      const payment: RefundPayment = {
        id: uid(),
        ref: seq('RFD', 'rfd'),
        rmaId: bb.id,
    ...
    creditBuyBack: async (id) => {
      const user = currentUser(); if (!user) return
      const bb = buyBacks.find(b => b.id === id)
      const allowed = canCreditBuyBack(bb)
      if (!allowed.ok) { showToast(allowed.error || 'Cannot add buy-back as credit', 'error'); return }
      const amount = buyBackStoreCreditAmount(bb)
      const at = now()
      let creditRef: string
      try {
        creditRef = await storeCtxRef.current!.allocateDocRef('CN')
      } catch {
        creditRef = seq('CN', 'inv')
      }
      const credit: CustomerCredit = {
        id: uid(),
        ref: creditRef,
        ...
        customerName: bb.customerName,
        sourceType: 'buyback',
        sourceBuyBackId: bb.id,
        sourceBuyBackRef: bb.ref,
        amount,
        balance: amount,
        status: 'available',
        createdAt: at,
        createdBy: user.name,
        notes: `Store credit from buy-back ${bb.ref}`,
        applications: [],
      }
      const journalLines = buildBuyBackStoreCreditJournalLines({
        buyBackRef: bb.ref,
        creditRef,
        customerName: bb.customerName,
        amount,
      }).map(line => accountLine(line.account, line.description, line.debit, line.credit))
      const debit = journalLines.reduce((s, l) => s + l.debit, 0)
      const creditTotal = journalLines.reduce((s, l) => s + l.credit, 0)
      const journal: JournalEntry = {
        id: uid(),
        ref: `JRN/${creditRef}`,
        date: at,
        source: 'manual',
        description: `Buy-back ${bb.ref} settled as store credit ${creditRef}`,
        status: 'posted',
        lines: journalLines,
        totalDebit: debit,
      ...
      setBuyBacks(p => p.map(b => b.id === id
        ...
        : b))
      addAuditLog('buyback_credit', bb.ref, `Store credit ${creditRef} for ${fmtKes(amount)} — ${bb.customerName}`)
      showToast(`${bb.ref} added as store credit ${creditRef} (${fmtKes(amount)})`)
    },
