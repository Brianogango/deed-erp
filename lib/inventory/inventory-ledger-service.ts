...
export async function validateGoodsReceiptAtomic(input: {
  ...
  return prisma.$transaction(async tx => {
    ...
    if(variance>0) journalLines.push({accountLabel:'6109 - Purchase Price Variance',label:'Purchase price variance',debit:variance,credit:0})
    else if(variance<0) journalLines.push({accountLabel:'6109 - Purchase Price Variance',label:'Purchase price variance',debit:0,credit:-variance})
