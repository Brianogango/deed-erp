-- Yesterday's Prisma GL client writes journal_entry_lines.analytic_account_id.
-- Production only applies database/migrations via deploy.yml, so that column
-- never landed. Delivery / POS COGS then fail with:
--   The column `analytic_account_id` of relation `journal_entry_lines`
