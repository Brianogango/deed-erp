...
const COMMAND_ACTIONS: Array<Pick<SearchResult, 'id' | 'title' | 'subtitle' | 'href' | 'module'> & { aliases: string[] }> = [
  {
    id: 'cmd-open-invoices',
    title: 'Open customer invoices',
    subtitle: 'Jump to customer invoices in Finance',
    href: '/finance?tab=invoices',
    module: 'accounting',
    aliases: ['invoice', 'invoices', 'customer invoice'],
  },
  {
...
const MODULE_SHORTCUTS: Array<{ id: string; title: string; subtitle: string; href: string; module: string; aliases: string[] }> = [
  ...
  { id: 'mod-sales', title: 'Sales', subtitle: 'Quotations and sales orders', href: '/sales', module: 'sales', aliases: ['sales', 'quotes', 'orders'] },
...
function highlight(text: string, query: string): React.ReactNode {
  ...
  return <>{text.slice(0, idx)}<mark className="bg-yellow-200 text-inherit rounded px-0.5">{text.slice(idx, idx + query.length)}</mark>{text.slice(idx + query.length)}</>
}
...
export default function GlobalSearch({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const handler = (event: KeyboardEvent) => {
      ...
      if (!dialog) return
      const focusable = Array.from(dialog.querySelectorAll<HTMLElement>('button:not([disabled]), input:not([disabled]), [href], [tabindex]:not([tabindex="-1"])')).filter(element => element.offsetParent !== null || element === document.activeElement)
      if (!focusable.length) {
        event.preventDefault()
        dialog.focus()
        return
  ...
  const results = useMemo((): SearchResult[] => {
    ...
    COMMAND_ACTIONS
      ...
      .filter(cmd => [cmd.title, cmd.subtitle, ...cmd.aliases].join(' ').toLowerCase().includes(q))
      .forEach(cmd => out.push({ ...cmd, type: 'command' }))

    MODULE_SHORTCUTS.forEach(mod => {
      if (allowedModules.has(mod.module) && [mod.title, mod.subtitle, ...mod.aliases].join(' ').toLowerCase().includes(q)) {
        out.push({ id: mod.id, type: 'module', title: mod.title, subtitle: mod.subtitle, href: mod.href, module: mod.module })
      }
    })
    ...
    if (/^open\s+repair\s+|^repair\s+rep-|^rep-/.test(q)) {
      ...
      if (targetRepair) out.unshift({ id: `cmd-open-${targetRepair.id}`, type: 'command', title: `Open repair ${targetRepair.ref}`, subtitle: `${targetRepair.customerName} · ${targetRepair.productName}`, href: `/repairs?id=${targetRepair.id}`, module: 'repair' })
    }

    if (!canSearchRecords) return out.slice(0, 30)

    if (allowedModules.has('contacts')) {
      ;(contacts || []).forEach(c => {
        if (c.name?.toLowerCase().includes(q) || c.phone?.toLowerCase().includes(q) || c.email?.toLowerCase().includes(q)) {
          out.push({
            id: c.id,
            type: 'contact',
            title: c.name,
            ...
            href: `/contacts?id=${encodeURIComponent(c.id)}`,
            module: 'contacts',
          })
        }
      })
    ...
    if (allowedModules.has('inventory')) {
      ;(products || []).forEach(p => {
    ...
    if (allowedModules.has('accounting')) {
      ;(invoices || []).filter(i => i.type === 'customer_invoice').forEach(inv => {
        if (inv.ref?.toLowerCase().includes(q) || inv.partnerName?.toLowerCase().includes(q)) {
          ...
          out.push({
            ...
            href: financeInvoicePath(inv.id),
            module: 'accounting',
          })
        }
      })
    ...
    if (allowedModules.has('repair')) {
      ;(repairs || []).forEach(r => {
        if (r.ref?.toLowerCase().includes(q) || r.customerName?.toLowerCase().includes(q) || r.productName?.toLowerCase().includes(q)) {
          out.push({ id: r.id, type: 'repair', title: r.ref, subtitle: `${r.customerName} · ${r.productName}`, badge: r.status?.replace(/_/g, ' '), badgeColor: '#F59E0B', href: `/repairs?id=${r.id}`, module: 'repair' })
        }
      })
    }

    if (allowedModules.has('purchase')) {
      ;(purchaseOrders || []).forEach(po => {
        if (po.ref?.toLowerCase().includes(q) || po.vendorName?.toLowerCase().includes(q)) {
          out.push({ id: po.id, type: 'purchase', title: po.ref, subtitle: `${po.vendorName} · KSh ${Math.round(po.total || 0).toLocaleString()}`, badge: po.status, badgeColor: po.status === 'received' ? '#10B981' : '#3B82F6', href: `/purchases?id=${encodeURIComponent(po.id)}`, module: 'purchase' })
        }
      })
    }

    if (allowedModules.has('sales')) {
      ;(quotes || []).forEach(q2 => {
        if (q2.quoteNumber?.toLowerCase().includes(q) || q2.client?.name?.toLowerCase().includes(q)) {
          out.push({ id: q2.id, type: 'quote', title: q2.quoteNumber, subtitle: `${q2.client?.name ?? ''} · KSh ${Math.round(q2.totalAmount || 0).toLocaleString()}`, badge: q2.status, badgeColor: q2.status === 'accepted' ? '#10B981' : '#06B6D4', href: `/sales?id=${encodeURIComponent(q2.id)}`, module: 'sales' })
        }
      })
    }
    ...
    if (allowedModules.has('hr')) {
      ;(employees || []).forEach(emp => {
        if (emp.fullName?.toLowerCase().includes(q) || emp.jobTitle?.toLowerCase().includes(q) || emp.departmentId?.toLowerCase().includes(q)) {
          out.push({ id: emp.id, type: 'employee', title: emp.fullName, subtitle: `${emp.jobTitle ?? ''} · ${emp.departmentId ?? ''}`, href: '/hr', module: 'hr' })
        }
      })
    }

    if (allowedModules.has('expenses')) {
      ;(expenses || []).forEach(exp => {
        if (exp.description?.toLowerCase().includes(q) || exp.submittedByName?.toLowerCase().includes(q)) {
          out.push({ id: exp.id, type: 'expense', title: exp.description || 'Expense', subtitle: `${exp.submittedByName} · KSh ${Math.round(exp.amount || 0).toLocaleString()}`, badge: exp.status, badgeColor: exp.status === 'approved' ? '#10B981' : '#F59E0B', href: '/expenses', module: 'expenses' })
        }
      })
    }
  ...
  const navigate = useCallback((result: SearchResult) => {
    trackUxEvent('search_navigate', { resultType: result.type, module: result.module, href: result.href, recordId: result.id })
    onClose()
  ...
  }, [open, flatResults, activeIdx, navigate])

  useEffect(() => {
    resultsRef.current?.querySelector(`[data-idx="${activeIdx}"]`)?.scrollIntoView({ block: 'nearest' })
  }, [activeIdx])

  if (!open) return null

  return (
    <div className="fixed inset-0 z-[9800] flex items-start justify-center overflow-y-auto px-4 py-4 sm:pt-[10vh]" style={{ animation: 'backdropIn 0.15s ease both' }} {...overlayDismiss}>
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="global-search-title" aria-describedby="global-search-description" tabIndex={-1} className="relative w-full max-w-2xl bg-[var(--bg-card)] rounded-2xl shadow-2xl border border-[var(--border)] overflow-hidden flex flex-col" style={{ maxHeight: '75vh', animation: 'modalIn 0.2s cubic-bezier(0.34,1.2,0.64,1) both' }} onClick={e => e.stopPropagation()}>
          ...
          {!query.trim() && (
            <div className="p-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {SHORTCUTS.filter(shortcut => shortcut.module === 'dashboard' || currentUser?.modules?.includes(shortcut.module)).map(s => (
                  <button type="button" key={s.key} onClick={() => { onClose(); router.push(s.href) }} className="flex items-center gap-2.5 px-3 py-3 rounded-xl bg-[var(--bg-surface)] hover:bg-[var(--bg-muted)] border border-[var(--border)] text-left transition-colors group active:scale-95">
              ...
              <p className="text-[10px] font-medium text-[var(--text-4)] mt-4 text-center">Start typing to run commands, jump modules, or search records</p>
            </div>
          )}
            ...
            return Object.entries(grouped).map(([type, items]) => {
              ...
              return (
                <div key={type} role="group" aria-label={`${cfg.label} results`}>
                  <div className="px-4 py-2 bg-[var(--bg-surface)]/60 border-b border-[var(--border-lt)] sticky top-0"><span className="text-[10px] font-black text-[var(--text-4)] uppercase tracking-widest">{cfg.label}s &nbsp;·&nbsp; {items!.length}</span></div>
                  {items!.map(result => {
                    const idx = globalIdx++
                    const isActive = idx === activeIdx
                    return (
                      <button type="button" key={`${result.type}:${result.id}`} id={`global-search-result-${idx}`} role="option" aria-selected={isActive} data-idx={idx} onClick={() => navigate(result)} onMouseEnter={() => setActiveIdx(idx)} className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-all border-b border-[var(--border-lt)] last:border-0 ${isActive ? 'bg-blue-500/[0.07]' : 'hover:bg-[var(--bg-surface)]'}`}>
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-base" style={{ background: cfg.bg, color: cfg.color }} aria-hidden="true">{cfg.icon}</div>
                        <div className="flex-1 min-w-0"><p className="text-[13px] font-bold text-[var(--text-1)] truncate">{highlight(result.title, query)}</p><p className="text-[11px] text-[var(--text-3)] truncate mt-0.5">{highlight(result.subtitle, query)}</p></div>
                        {result.badge && <span className="shrink-0 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider" style={{ background: `${result.badgeColor}18`, color: result.badgeColor, border: `1px solid ${result.badgeColor}30` }}>{result.badge}</span>}
                        {isActive && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="text-[var(--text-4)] shrink-0" aria-hidden="true"><path d="M5 12h14M12 5l7 7-7 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                      </button>
                    )
                  })}
                </div>
