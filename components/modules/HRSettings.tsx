  ...
  return (
    <div className="max-w-6xl mx-auto pb-16">
      ...
      <div className="mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0">
        <div>
          <h2 className="text-15 font-bold text-gray-900 leading-tight">System Settings</h2>
                    ...
                    return (
                      <div key={user.id} className="p-4">
                        ...
                        <div className="flex gap-2">
                          <button className="flex-1 text-11 font-medium py-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-[var(--ink-navy)] border border-blue-100 cursor-pointer transition-colors" onClick={() => { const u = users.find(x => x.id === user.id); if (!u) return; setUserForm({ id: u.id, username: u.username, name: u.name, role: u.role, modules: Array.isArray(u.modules) ? [...u.modules] : [], active: u.active, password: '', employeeId: u.employeeId ?? '' }); setShowUserModal(true) }}>Edit</button>
                          {user.lockedUntil && new Date(user.lockedUntil).getTime() > Date.now() && (
                            <button className="flex-1 text-11 font-medium py-1.5 rounded-lg bg-orange-50 hover:bg-orange-100 text-orange-600 border border-orange-100 cursor-pointer transition-colors" onClick={() => { void unlockUser(user.id) }}>Unlock</button>
                          )}
                          <button className={`flex-1 text-11 font-medium py-1.5 rounded-lg border transition-colors ${user.id === currentUserId ? 'opacity-40 cursor-not-allowed bg-gray-50 text-gray-400 border-gray-100' : 'bg-red-50 hover:bg-red-100 text-red-600 border-red-100 cursor-pointer'}`} disabled={user.id === currentUserId} onClick={() => { void removeUser(user.id) }}>Delete</button>
                        </div>
                      </div>
                    )
                  })}
                    ...
                    {users.map(user => {
                      ...
                      return (
                        <div key={user.id} className="table-row">
                          ...
                          <span className="flex gap-1.5">
                            <button className="text-10 font-medium px-2.5 py-1 rounded-lg bg-blue-50 hover:bg-blue-100 text-[var(--ink-navy)] border border-blue-100 cursor-pointer transition-colors" onClick={() => { const u = users.find(x => x.id === user.id); if (!u) return; setUserForm({ id: u.id, username: u.username, name: u.name, role: u.role, modules: Array.isArray(u.modules) ? [...u.modules] : [], active: u.active, password: '', employeeId: u.employeeId ?? '' }); setShowUserModal(true) }}>Edit</button>
                            {user.lockedUntil && new Date(user.lockedUntil).getTime() > Date.now() && (
                              <button className="text-10 font-medium px-2.5 py-1 rounded-lg bg-orange-50 hover:bg-orange-100 text-orange-600 border border-orange-100 cursor-pointer transition-colors" onClick={() => { void unlockUser(user.id) }}>Unlock</button>
                            )}
                            <button className={`text-10 font-medium px-2.5 py-1 rounded-lg border transition-colors ${user.id === currentUserId ? 'opacity-40 cursor-not-allowed bg-gray-50 text-gray-400 border-gray-100' : 'bg-red-50 hover:bg-red-100 text-red-600 border-red-100 cursor-pointer'}`} disabled={user.id === currentUserId} onClick={() => { void removeUser(user.id) }}>Del</button>
                          </span>
                        </div>
                      )
                    })}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    ...
                    { role: 'Admin Officer',     color: 'var(--ink-navy)', bg: '#EEF2FF', border: '#C7D2FE', desc: 'Process, master-data, and workflow control. No payment posting or accounting.' },
                    { role: 'Finance Officer',   color: 'var(--warning)', bg: '#FFFBEB', border: '#FDE68A', desc: 'Invoicing, bills, payments, bank/cash, tax, reconciliation, and financial reports.' },
            ...
            {userForm.id ? (
              ...
              <div className="sm:col-span-2">
                ...
                {selectedProvisionEmployee && (
                  <p className="mt-2 text-11 text-gray-500 bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
