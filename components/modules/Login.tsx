...
export default function Login() {
  ...
  const resolveDestination = (defaultModule?: string) => {
    ...
    if (typeof defaultModule === 'string' && defaultModule && defaultModule !== 'dashboard') {
      return `/${defaultModule}`
    }
    return '/'
  }

  const beginMfa = async (enroll: boolean) => {
    setCode('')
    if (!enroll) {
      setPhase('verify')
      return
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      window.location.href = resolveDestination(data.defaultModule)
    } catch (error) {
      setToast({
        msg: error instanceof Error ? error.message : 'Authentication service is unavailable',
        type: 'error',
      })
    } finally {
      setPending(false)
  ...
  const handleMfaSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
        setToast({ msg: data.message || 'Invalid authenticator code', type: 'error' })
        setCode('')
        return
      }
      ...
      setToast({ msg: 'Verification service is unavailable', type: 'error' })
      setCode('')
    } finally {
      setPending(false)
    }
    ...
    <div className="relative min-h-screen w-full overflow-x-hidden overflow-y-auto">
      ...
      <div className="relative z-10 flex min-h-screen w-full items-center justify-center px-5 py-10 md:justify-start md:px-[6vw] lg:px-[8vw]">
        <div className="w-full max-w-[430px] rounded-[22px] border border-white/60 bg-white/95 px-7 py-9 shadow-2xl backdrop-blur-xl sm:px-9"
          ...
          {/* Heading / welcome — outer positioning intentionally unchanged */}
          <div className="mb-6">
            <h1 className="text-[24px] font-bold tracking-[-0.01em] text-slate-900">
              {phase === 'credentials' ? 'Sign in' : phase === 'enroll' ? 'Protect your account' : 'Verify your identity'}
            </h1>
            <p className="mt-1 text-[13px] leading-5 text-slate-500">
              {phase === 'credentials'
                ? 'Welcome back — sign in to your workspace.'
                : phase === 'enroll'
                  ? 'Scan the authenticator QR code, then enter the six-digit PIN below.'
          ...
          {phase === 'credentials' ? (
            ...
            <form className="space-y-4" onSubmit={handleMfaSubmit} noValidate>
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  {qrDataUrl ? (
                    <img
                      ...
                      className="mx-auto block h-[190px] w-[190px] rounded-xl bg-white p-2 object-contain"
                    />
                  ) : (
                    <div className="flex h-[190px] items-center justify-center rounded-xl bg-white text-[12px] text-slate-500">
                      Loading authenticator QR code…
                    </div>
                  )}
                  {manualKey && (
                    <div className="mt-3">
                      <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">Manual setup key</p>
                      <code className="mt-1 block break-all rounded-xl border border-slate-200 bg-white px-3 py-2 text-[11px] text-slate-700">
                        {manualKey}
                      </code>
                    </div>
                  )}
              ...
              <button
                type="button"
                onClick={startOver}
                className="w-full rounded-2xl border border-slate-200 bg-white py-3 text-[12px] font-semibold text-slate-500 transition hover:bg-slate-50 hover:text-slate-700"
              >
                Back to sign in
              </button>
            </form>
