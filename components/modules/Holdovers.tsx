...
function DaysTag({ h }: { h: Holdover }) {
  if (h.returnedDate) {
    ...
    return <span className="text-11 text-[var(--text-4)]">{days}d loan</span>
  }
  const issued = new Date(h.issuedDate)
  ...
  return (
    <span className={`text-11 font-semibold ${overdue ? 'text-red-500' : 'text-[var(--text-3)]'}`}>
      ...
      <div style={{ animation: 'modalIn 0.2s ease both' }}
        ...
        <div className="px-5 py-4 border-b border-[var(--border-lt)] flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-11 text-[var(--text-4)] mt-0.5">Step {step} of 2 — {step === 1 ? 'Client & Device' : 'Loan Details'}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {step === 1 && (
            <>
              {/* Client */}
              <div>
                <p className="text-11 font-bold text-[var(--text-4)] uppercase tracking-wider mb-2">Client</p>
                    ...
                    <div className="absolute top-full mt-1 left-0 right-0 z-10 bg-[var(--bg-card)] border border-[var(--border)] rounded-xl shadow-xl overflow-hidden">
                      {filteredContacts.map(c => (
                        ...
                        >
                          ...
                          <p className="text-11 text-[var(--text-4)]">{c.phone}</p>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Full Name *</label>
                    <input value={clientName} onChange={e => setClientName(e.target.value)} placeholder="John Doe"
                      className="form-input" />
                  </div>
                  <div>
                    <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Phone *</label>
                    <input value={clientPhone} onChange={e => setClientPhone(e.target.value)} placeholder="07XX XXX XXX"
                      className="form-input" />
                  </div>
                </div>
                <div className="mt-3">
                  <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">National ID / Passport</label>
              ...
              {/* Device */}
              <div>
                <p className="text-11 font-bold text-[var(--text-4)] uppercase tracking-wider mb-2">Device to Issue</p>
                <div className="space-y-3">
                  <div>
                    <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Product *</label>
                    ...
                    {loanableProducts.length === 0 && (
                      <p className="text-11 text-amber-500 mt-1">No serialised devices with available stock.</p>
                    )}
                  </div>

                  {productId && (
                    <div style={{ animation: 'fadeIn 0.18s ease both' }}>
                      <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Serial Number *</label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Condition when given</label>
                    ...
                    <div>
                      <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Accessories</label>
            ...
            <div style={{ animation: 'fadeIn 0.2s ease both' }} className="space-y-4">
              ...
              <div className="p-3 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-11 text-[var(--text-4)]">S/N: {selectedSerial?.serial} · to {clientName}</p>
                </div>
              </div>

              <div>
                <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Purpose *</label>
              ...
              {purpose === 'repair_loaner' && (
                <div style={{ animation: 'fadeIn 0.18s ease both' }}>
                  <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Link to Repair Job</label>
              ...
              <div>
                <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Note (optional)</label>
                <input value={purposeNote} onChange={e => setPurposeNote(e.target.value)} placeholder="Any additional details…"
                  className="form-input" />
              </div>

              <div>
                <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Authorized By *</label>
                ...
                <p className="text-10 text-[var(--text-4)] mt-1">The manager or staff member who approved this loan.</p>
              </div>

              <div>
                <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Expected Return Date *</label>
      ...
      <div style={{ animation: 'confirmIn 0.2s cubic-bezier(0.34,1.4,0.64,1) both' }}
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="p-3 rounded-xl bg-[var(--bg-surface)] border border-[var(--border)]">
            ...
            <p className="text-11 text-[var(--text-4)]">S/N: {holdover.serialNumber}</p>
            <p className="text-11 text-[var(--text-4)]">Issued to: {holdover.clientName} · {holdover.ref}</p>
            {holdover.accessories && <p className="text-11 text-[var(--text-4)] mt-0.5">Accessories: {holdover.accessories}</p>}
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Return Condition</label>
            ...
            <div>
              <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Return to</label>
          ...
          <div>
            <label className="text-11 font-semibold text-[var(--text-3)] block mb-1">Return Notes</label>
          ...
          {condition === 'damaged' && (
            <div style={{ animation: 'fadeIn 0.18s ease both' }}
              ...
              <p className="text-11 text-red-400 leading-relaxed">Device returned damaged. Consider creating a repair job after confirming.</p>
            </div>
          )}
        </div>
    ...
    <div className="fixed inset-0 z-[9100] flex items-end sm:items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(4px)' }}>
      <div style={{ animation: 'modalIn 0.2s ease both' }}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="p-4 rounded-xl bg-[var(--bg-surface)] border border-[var(--border)]">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-12 text-[var(--text-3)] mt-0.5">S/N: <span className="font-mono font-semibold">{holdover.serialNumber}</span></p>
                {holdover.accessories && <p className="text-12 text-[var(--text-4)] mt-0.5">With: {holdover.accessories}</p>}
              </div>
              <div className="text-right text-11">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-xl bg-[var(--bg-surface)] border border-[var(--border)]">
              <p className="text-10 font-bold text-[var(--text-4)] uppercase tracking-wider mb-2">Client</p>
              <p className="font-bold text-[var(--text-1)] text-sm">{holdover.clientName}</p>
              <p className="text-12 text-[var(--text-3)]">{holdover.clientPhone}</p>
              {holdover.clientIdNo && <p className="text-11 text-[var(--text-4)] mt-0.5">ID: {holdover.clientIdNo}</p>}
            </div>
            <div className="p-3 rounded-xl bg-[var(--bg-surface)] border border-[var(--border)]">
              <p className="text-10 font-bold text-[var(--text-4)] uppercase tracking-wider mb-2">Purpose</p>
              <p className="font-bold text-[var(--text-1)] text-sm">{PURPOSE_LABELS[holdover.purpose]}</p>
              {holdover.purposeNote && <p className="text-12 text-[var(--text-3)] mt-0.5 line-clamp-2">{holdover.purposeNote}</p>}
              {holdover.linkedRepairRef && (
                <p className="text-11 text-blue-500 mt-1">Repair: {holdover.linkedRepairRef}</p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={label} className="p-3 rounded-xl bg-[var(--bg-surface)] border border-[var(--border)] text-center">
                <p className="text-10 font-bold text-[var(--text-4)] uppercase tracking-wider mb-1">{label}</p>
          ...
          {holdover.returnedDate && (
            <div style={{ animation: 'fadeIn 0.2s ease both' }} className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
              <p className="text-11 font-bold text-emerald-400 mb-1">Return recorded</p>
              ...
              {holdover.returnNotes && <p className="text-11 text-[var(--text-3)] mt-1">{holdover.returnNotes}</p>}
            </div>
          )}

          <div className="flex items-center gap-4 text-10 text-[var(--text-4)]">
    ...
    <div className="mod-page">
      ...
      <div className="mod-header">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-12 text-[var(--text-4)] mt-0.5">Device loans & temporary issue log</p>
        </div>
        <button
      ...
      {/* Stats */}
      <div className="px-6 py-3 stat-grid-4 border-b border-[var(--border-lt)]">
        {[
          ...
          <div key={s.label} className={`${s.bg} rounded-xl p-3 border border-[var(--border)]`}>
            ...
            <p className="text-11 font-semibold text-[var(--text-4)] mt-0.5">{s.label}</p>
          </div>
        ))}
      </div>
        ...
        <div className="flex gap-1.5 overflow-x-auto scrollbar-none flex-shrink-0">
          {(['all', 'active', 'overdue', 'returned'] as const).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-12 font-semibold transition-all cursor-pointer capitalize ${
                filter === f
                ...
                <thead>
                  <tr className="border-b border-[var(--border-lt)]">
                    {['Ref', 'Client', 'Device / Serial', 'Purpose', 'Issued', 'Return By', 'Duration', 'Status', ''].map(h => (
                      <th key={h} className="text-left px-5 py-3 text-10 font-bold text-[var(--text-4)] uppercase tracking-wider">{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {filtered.map((h, i) => (
                    <tr key={h.id} onClick={() => setDetail(h)}
                      ...
                      <td className="px-5 py-3.5">
                        <span className="font-mono text-12 font-bold text-blue-500">{h.ref}</span>
                      </td>
                      <td className="px-5 py-3.5">
                        <p className="text-sm font-semibold text-[var(--text-1)]">{h.clientName}</p>
                        <p className="text-11 text-[var(--text-4)]">{h.clientPhone}</p>
                      </td>
                      <td className="px-5 py-3.5">
                        <p className="text-sm font-semibold text-[var(--text-1)]">{h.productName}</p>
                        <p className="text-11 font-mono text-[var(--text-4)]">{h.serialNumber}</p>
                      </td>
                      <td className="px-5 py-3.5">
                        <p className="text-12 text-[var(--text-2)]">{PURPOSE_LABELS[h.purpose]}</p>
                        {h.linkedRepairRef && <p className="text-11 text-blue-500">{h.linkedRepairRef}</p>}
                      </td>
                      <td className="px-5 py-3.5 text-12 text-[var(--text-3)]">{fmt(h.issuedDate)}</td>
                      <td className="px-5 py-3.5 text-12 text-[var(--text-3)]">{fmt(h.expectedReturnDate)}</td>
                      ...
                      <td className="px-5 py-3.5">
                        {h.status !== 'returned' && (
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg bg-emerald-600/10 hover:bg-emerald-600/20 text-emerald-500 text-11 font-bold border border-emerald-500/20 transition-colors cursor-pointer opacity-0 group-hover:opacity-100"
                          >
                            Return
                          </button>
                        )}
              ...
              {filtered.map((h, i) => (
                <div key={h.id} onClick={() => setDetail(h)}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-11 text-[var(--text-3)]">
                    ...
                    {h.status !== 'returned' && (
                      <button
                        onClick={e => { e.stopPropagation(); setReturning(h) }}
                        className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-11 font-bold cursor-pointer"
                      >
                        Return
                      </button>
                    )}
