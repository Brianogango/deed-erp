    ...
    <div className="mod-page">
      ...
      <div className="mod-header">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-10 text-text-3 mt-0.5">Submit &amp; track expense claims</p>
          </div>
        </div>
      ...
      <div className="px-4 py-3 stat-grid-4 border-b border-border-lt bg-surface">
        {isFinance ? (<>
          ...
          <StatCard label="Pending Amount"     value={fmtKes(totalPendingAmt)} sub="to review"          color="var(--ink-navy)" icon={<Fa icon={faMoneyBillWave} />} />
          ...
          <StatCard label="Total Submitted"  value={fmtKes(myTotal)}      color="var(--ink-navy)" icon={<Fa icon={faClipboardList} />} />
      ...
      <div className="mod-tabs">
        {[
          ...
          <button key={t.key} onClick={() => setTab(t.key)} className={`mod-tab ${tab === t.key ? 'active' : ''}`}>
            {t.label}
            {t.count > 0 && <span className="ml-1.5 badge badge-gray text-9">{t.count}</span>}
          </button>
        ))}
      </div>
              ...
              {([
                ...
                <button key={f.value} onClick={() => setReviewStatus(f.value)}
                  style={{
                    ...
                    background:  reviewStatus === f.value ? 'var(--ink-navy)' : 'var(--bg-muted)',
                    color:       reviewStatus === f.value ? '#fff'    : 'var(--text-3)',
                    borderColor: reviewStatus === f.value ? 'var(--ink-navy)' : 'var(--border)',
                    fontWeight:  reviewStatus === f.value ? 600 : 400,
                  }}>
                  {f.label}
                </button>
              ))}
              <span className="text-10 text-t3 ml-2">By:</span>
              <select className="form-input text-11 py-1" value={reviewUser} onChange={e => setReviewUser(e.target.value)} style={{ minWidth: 130 }}>
          ...
          <div className="modal-box w-full max-w-lg" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-10 mt-0.5" style={{ color: 'var(--text-3)' }}>
                  Submitting as <span className="font-semibold" style={{ color: 'var(--ink-navy)' }}>{currentUser?.name ?? '—'}</span>
                </p>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-11 font-semibold text-t2 block mb-1">Expense Date *</label>
                  <input type="date" className="form-input w-full text-12" value={form.expenseDate}
              ...
              {/* Description */}
              <div>
                <label className="text-11 font-semibold text-t2 block mb-1">Description *</label>
              ...
              {/* Amount */}
              <div>
                <label className="text-11 font-semibold text-t2 block mb-1">Amount (KSh) *</label>
                <input type="number" className="form-input w-full text-12" placeholder="0.00"
                  value={form.amount}
              ...
              {/* Payment method */}
              <div>
                <label className="text-11 font-semibold text-t2 block mb-2">How was it paid? *</label>
                  ...
                  {PAYMENT_METHODS.map(pm => {
                    ...
                    return (
                      <button key={pm.value} onClick={() => setForm(f => ({ ...f, paymentMethod: pm.value }))}
                        ...
                        <p className="text-11 font-semibold" style={{ color: active ? '#14204F' : '#374151' }}>{pm.label}</p>
                        <p className="text-10" style={{ color: active ? '#00B0D7' : '#9CA3AF' }}>{pm.desc}</p>
                        {pm.isReimbursable && (
                          <span style={{ fontSize: 9, background: '#FEF3C7', color: 'var(--warning)', borderRadius: 20, padding: '1px 6px', fontWeight: 600, marginTop: 3, display: 'inline-block' }}>
                            Reimbursable
                          </span>
                        )}
                      </button>
              ...
              <div>
                ...
                <div
                  ...
                  style={{
                    border: `2px dashed ${dragOver ? 'var(--ink-navy)' : receiptFile ? '#10B981' : '#D1D5DB'}`,
                  ...
                  {isScanning ? (
                    <div className="flex flex-col items-center justify-center py-3 gap-2">
                      <svg className="h-6 w-6 animate-spin" style={{ color: 'var(--ink-navy)' }} viewBox="0 0 24 24" fill="none">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                      </svg>
                      <p className="text-11 font-bold text-t1 mt-1">Reading receipt with Deed OCR...</p>
                      <p className="text-10 text-t3">Your own scanner is extracting amount, date & description</p>
                    </div>
                  ) : receiptFile ? (
                    <div>
                      ...
                      <p className="text-11 font-semibold text-green-700">{receiptFile.name}</p>
                      <p className="text-10 text-t3 mt-0.5">{formatSize(receiptFile.size)} · Click to change</p>
                      {receiptFile.type.startsWith('image/') && (
                        <button
                          type="button"
                          className="btn-outline text-10 py-1 px-3 mt-2"
                    ...
                    <div>
                      ...
                      <p className="text-11 text-t2 font-medium">Drop receipt here or click to browse</p>
                      <p className="text-10 text-t3 mt-0.5">JPG, PNG, or WebP images scan with Deed OCR — max 8 MB; PDF/manual upload max 10 MB</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <input className="form-input w-full text-12" placeholder="Any additional context..."
                  value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
              </div>
            </div>

            <div className="flex gap-2 mt-4 justify-end">
              <button className="btn-outline text-11 py-2 px-4" onClick={() => setShowSubmit(false)}>Cancel</button>
              <button className="btn-primary text-11 py-2 px-4" onClick={handleSubmit}>Submit Expense</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="modal-box w-full max-w-md" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-11 text-t3">{exp.ref} · {exp.submittedByName}</p>
                </div>
              ...
              <div className="rounded-xl p-3 mb-4 space-y-1.5" style={{ background: '#F9FAFB', border: '1px solid #F3F4F6' }}>
                ...
                <div className="flex justify-between text-12">
                  ...
                  <span>{pmLabel(exp.paymentMethod)}{isReimbursable(exp.paymentMethod) && <span className="ml-1 text-10 text-amber-700 font-semibold">(Reimbursable)</span>}</span>
                </div>
                <div className="flex justify-between text-12">
                  <span className="text-t3">Description</span>
                  <span className="font-medium text-right ml-4 max-w-[220px]">{exp.description}</span>
                </div>
                <div className="flex justify-between text-12 pt-1 border-t" style={{ borderColor: '#E5E7EB' }}>
                  <span className="font-bold text-t1">Amount</span>
                  <span className="font-bold text-base" style={{ color: 'var(--ink-navy)' }}>{fmtKes(exp.amount)}</span>
                </div>
              </div>

              {exp.receiptFileName && (
                <button onClick={() => { openReceiptPreview(exp); setReviewingId(null) }}
                  style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--ink-navy)', background: '#E8F3FA', border: '1px solid #A8D4E8', borderRadius: 8, padding: '6px 12px', cursor: 'pointer', marginBottom: 12 }}>
              ...
              {exp.reviewNotes && !canReview && (
                <div className="mb-3 p-3 rounded-lg" style={{ background: '#FEF3C7', border: '1px solid #FDE68A' }}>
                  <p className="text-10 font-semibold uppercase tracking-wider text-t3 mb-1">Review Notes</p>
                  <p className="text-xs" style={{ color: 'var(--warning)' }}>{exp.reviewNotes}</p>
                </div>
              )}
              ...
              {exp.status === 'reimbursed' && (exp.reimbursementMethod || exp.reimbursementBankAccount || exp.reimbursementReference) && (
                <div className="mb-3 p-3 rounded-lg" style={{ background: '#ECFEFF', border: '1px solid #A5F3FC' }}>
                  <p className="text-10 font-semibold uppercase tracking-wider text-t3 mb-2">Reimbursement Details</p>
              ...
              {canReview && (
                <div className="mb-3">
                  <label className="text-11 font-semibold text-t2 block mb-1">Review Notes (optional)</label>
                  <textarea className="form-input w-full text-12" rows={2}
              ...
              <div className="flex gap-2 justify-end">
                {canReview ? (
                  <>
                    <button onClick={() => setReviewingId(null)} className="btn-outline text-11 py-2 px-4">Cancel</button>
                    ...
                    <button onClick={() => { reviewExpense(reviewingId, true, reviewNotes); setReviewingId(null) }}
                      className="btn-primary text-11 py-2 px-4" style={{ background: '#10B981' }}>
                      Approve
                    </button>
                  </>
                ) : (
                  <button onClick={() => setReviewingId(null)} className="btn-outline text-11 py-2 px-4">Close</button>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="modal-overlay" onClick={() => setReimbursingId(null)}>
            <div className="modal-box w-full max-w-sm" onClick={e => e.stopPropagation()}>
              ...
              <div className="rounded-xl p-3 mb-4 text-center" style={{ background: '#E8F3FA', border: '1px solid #A8D4E8' }}>
                <p className="text-10 text-t3 mb-1">Amount to reimburse to {exp.submittedByName}</p>
                <p className="text-2xl font-bold" style={{ color: 'var(--ink-navy)' }}>{fmtKes(exp.amount)}</p>
                <p className="text-10 text-t3 mt-1">{catLabel(exp.category)} · {fmtDate(exp.expenseDate)}</p>
              </div>

              <div className="space-y-3 mb-4">
                <div>
                  <label className="text-11 font-semibold text-t2 block mb-1">Bank Account</label>
                  <select className="form-input w-full text-12" value={reimburseBankAccountId} onChange={e => setReimburseBankAccountId(e.target.value)}>
                ...
                <div>
                  ...
                  <select className="form-input w-full text-12" value={reimburseMethod} onChange={e => setReimburseMethod(e.target.value)}>
                ...
                <div>
                  <label className="text-11 font-semibold text-t2 block mb-1">{reimburseMethod === 'cheque' ? 'Cheque Number' : 'Payment Reference'}</label>
                  <input className="form-input w-full text-12" placeholder={reimburseMethod === 'cheque' ? 'e.g. 000123' : 'e.g. M-Pesa ref QGH123XY'}
                    value={reimburseReference} onChange={e => setReimburseReference(e.target.value)} />
                </div>
                <div>
                  <label className="text-11 font-semibold text-t2 block mb-1">Reimbursement Note (optional)</label>
                  <textarea className="form-input w-full text-12" rows={2} placeholder="Any note about the reimbursement..."
                    value={reimburseNote} onChange={e => setReimburseNote(e.target.value)} />
                </div>
              </div>

              <div className="flex gap-2 justify-end">
                <button onClick={() => { setReimbursingId(null); setReimburseReference(''); setReimburseBankAccountId('') }} className="btn-outline text-11 py-2 px-4">Cancel</button>
                <button onClick={() => {
                  ...
                  className="btn-primary text-11 py-2 px-4" style={{ background: '#00B0D7' }}>
      ...
      {previewExp && (
        <div className="modal-overlay" onClick={closeReceiptPreview}>
          <div className="modal-box w-full max-w-2xl" onClick={e => e.stopPropagation()} style={{ display: 'flex', flexDirection: 'column' }}>
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-sm font-bold text-t1">{previewExp.ref} · Receipt</h3>
                <p className="text-10 text-t3">{previewExp.receiptFileName} · {previewExp.receiptFileSize ? formatSize(previewExp.receiptFileSize) : ''}</p>
              </div>
              <div className="flex items-center gap-2">
                {previewUrl && (
                  <a href={previewUrl} download={previewExp.receiptFileName ?? 'receipt'}
                    className="btn-outline text-11 py-1.5 px-3" style={{ textDecoration: 'none' }}>
                    Download
                  </a>
                )}
              ...
              {previewLoading ? (
                ...
                previewExp.receiptFileType?.startsWith('image/') ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-48 text-t3 text-sm gap-2">
                    ...
                    <a href={previewUrl} download={previewExp.receiptFileName} className="btn-primary text-11 py-2 px-4" style={{ textDecoration: 'none' }}>Download to view</a>
                  </div>
                )
              ) : (
                <div className="flex flex-col items-center justify-center h-48 text-t3 text-sm gap-2">
                  <span style={{ fontSize: 40 }}>⚠️</span>
                  <p className="text-12">Receipt could not be loaded</p>
                </div>
              )}
            </div>
          </div>
        ...
        <thead>
          <tr className="bg-[var(--bg-surface)] border-b border-[var(--border-lt)]">
            {[
              ...
              <th key={h} className="px-6 py-4 text-left text-10 font-bold text-[var(--text-4)] uppercase tracking-wider whitespace-nowrap">{h}</th>
            ))}
          </tr>
        </thead>
          ...
          {rows.map((exp) => (
            <tr key={exp.id} className="hover:bg-[var(--bg-surface)] transition-colors">
              <td className="px-6 py-4">
                <span className="font-mono text-11 font-bold text-primary-600">{exp.ref}</span>
              </td>
              ...
              <td className="px-6 py-4" style={{ maxWidth: 250 }}>
                ...
                {exp.notes && <p className="text-10 text-[var(--text-4)] truncate mt-0.5">{exp.notes}</p>}
              </td>
              {showSubmitter && (
              ...
              <td className="px-6 py-4 text-[var(--text-3)] whitespace-nowrap font-medium">
                ...
                {isReimbursable(exp.paymentMethod) && (
                  <span className="block text-9 text-amber-600 font-bold uppercase mt-0.5">Reimbursable</span>
                )}
              </td>
              <td className="px-6 py-4">
                <StatusBadge status={exp.status} />
                {exp.reviewNotes && (
                  <p className="text-10 text-[var(--text-4)] mt-1 italic truncate max-w-[120px]" title={exp.reviewNotes}>{exp.reviewNotes}</p>
                )}
                {exp.status === 'reimbursed' && exp.reimbursementReference && (
                  <p className="text-10 text-cyan-700 mt-1 truncate max-w-[120px]" title={exp.reimbursementReference}>Paid: {exp.reimbursementReference}</p>
                )}
              </td>
              <td className="px-6 py-4">
                {exp.receiptFileName ? (
              ...
              <td className="px-6 py-4">
                <div className="flex items-center gap-2">
                  {onReview && exp.status === 'submitted' && (
                    <button className="btn-primary text-10 py-1.5 px-3" onClick={() => onReview(exp)}>Review</button>
                  )}
                  {onReimburse && exp.status === 'approved' && isReimbursable(exp.paymentMethod) && (
                    <button className="btn-primary text-10 py-1.5 px-3 bg-cyan-600 hover:bg-cyan-700" onClick={() => onReimburse(exp)}>Reimburse</button>
                  )}
                  {onView && (
                    <button className="btn-secondary text-10 py-1.5 px-3" onClick={() => onView(exp)}>View</button>
                  )}
                </div>
              </td>
            </tr>
