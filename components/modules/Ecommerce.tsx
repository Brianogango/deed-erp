  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="kpi-grid">
        ...
        <StatCard label="Out of Stock"    value={outOfStock}             sub="not showing online" color="#EF4444" icon={<Fa icon={faTriangleExclamation} />} onClick={() => setModule('inventory')} />
          ...
          <>
            ...
            {onlineOrders.map(o => (
              <div key={o.id} className="table-row" style={{ gridTemplateColumns: '80px 1.3fr 1.5fr 90px 70px 70px' }}>
                ...
                <button className="btn-outline text-[10px] py-0.5 px-2" onClick={() => setModule('sales')}>→ Sales</button>
              </div>
            ))}
          </>
        )}
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { icon: '📦', title: 'Manage Products',  desc: 'Add, edit, set prices & stock',        action: () => setModule('inventory'),  color: '#F59E0B' },
          { icon: '💼', title: 'Process as Sale',  desc: 'Convert online orders to sale orders', action: () => setModule('sales'),      color: '#8B5CF6' },
          { icon: '📊', title: 'View Accounting',  desc: 'Online revenue in accounting',         action: () => setModule('accounting'), color: '#10B981' },
        ].map(c => (
