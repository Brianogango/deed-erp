            ...
            : vendorBills.map(b => {
                ...
                return (
                  <div key={b.id} className="table-row" style={{ gridTemplateColumns: '90px 90px 1.4fr 100px 85px 85px 85px 90px' }}>
                    <span className="font-mono text-11 font-semibold" style={{ color: 'var(--ink-navy)' }}>{b.ref}</span>
                    <span className="font-mono text-10 text-t3">{linkedPORef}</span>
                    <span className="text-t1">{b.partnerName}</span>
                    <span className="text-11 text-t3">{fmtDate(b.dueDate)}</span>
                    <span className="font-mono text-11 text-t1">{fmtKes(b.total)}</span>
                    <span className="font-mono text-11" style={{ color: '#10B981' }}>{fmtKes(b.amountPaid)}</span>
                    <span className="font-mono text-11" style={{ color: isPaid ? '#10B981' : '#EF4444' }}>{fmtKes(outstanding)}</span>
                    <div className="flex items-center gap-1.5">
                      {b.status === 'draft' && canValidateBills && (
                        <button className="btn-primary text-9 py-0.5 px-2" style={{ background: '#10B981' }}
                      ...
                      {(b.status === 'posted' || b.status === 'partially_paid' || b.status === 'overdue') && outstanding > 0 && (
                        <span className="text-9 text-[var(--text-4)] italic">Pay via Finance</span>
                      )}
