    ...
    <div className="card p-5 overflow-hidden">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-10 text-[var(--text-3)]">Frequency of orders by day and hour</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-10 text-[var(--text-4)]">Less</span>
          <div className="flex gap-0.5">
            {[0.2, 0.4, 0.6, 0.8, 1].map(o => (
              <div key={o} className="w-2.5 h-2.5 rounded-sm" style={{ background: `rgba(27, 39, 98, ${o})` }} />
            ))}
          </div>
          <span className="text-10 text-[var(--text-4)]">More</span>
        </div>
      </div>
        ...
        <div className="min-w-[600px]">
          <div className="flex">
            ...
            <div className="flex-1 flex justify-between px-2 mb-2">
              {['12am', '4am', '8am', '12pm', '4pm', '8pm'].map(h => (
                <span key={h} className="text-9 text-[var(--text-4)] font-medium uppercase">{h}</span>
              ))}
            </div>
          </div>
          ...
          {data.map((dayData, dayIdx) => (
            <div key={days[dayIdx]} className="flex items-center mb-1">
              <div className="w-10 text-10 font-bold text-[var(--text-3)] flex-shrink-0">{days[dayIdx]}</div>
                ...
                {dayData.map((val, hourIdx) => (
                  ...
                  >
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-10">
                      <div className="bg-gray-800 text-white text-9 px-2 py-1 rounded shadow-lg whitespace-nowrap">
