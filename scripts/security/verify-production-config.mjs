...
const requiredSecret = (name, min = 32) => {
  const value = String(env[name] || '')
  if (!value) failures.push(`${name} is missing`)
  else if (value.length < min) failures.push(`${name} is shorter than ${min} characters`)
  else if (/REPLACE_WITH|changeme|password|secret/i.test(value)) failures.push(`${name} still looks like a placeholder/default`)
  return value
}
...
if (env.MFA_ENFORCE_PRIVILEGED !== 'true') failures.push('MFA_ENFORCE_PRIVILEGED must be true in production')
...
if (!dbRaw) {
  ...
  try {
    ...
    if (!localHosts.has(host) && !privateV4) warnings.push(`Database host ${host} is not localhost/private; confirm network ACL and TLS explicitly`)
    ...
    if (/^(postgres|root)$/i.test(decodeURIComponent(db.username))) failures.push('ERP must not connect using the PostgreSQL superuser/root account')
  } catch {
    failures.push('DATABASE_URL/POSTGRES_URL is invalid')
  }
}

const distinctSecrets = [auth, mfaKey, mfaChallenge].filter(Boolean)
if (new Set(distinctSecrets).size !== distinctSecrets.length) failures.push('Auth and MFA secrets must be distinct values')

if (String(env.NODE_ENV || '') !== 'production') warnings.push('NODE_ENV is not production for this verification run')
if (env.SMTP_REQUIRE_TLS !== 'true' && env.EMAIL_PROVIDER === 'smtp') warnings.push('SMTP_REQUIRE_TLS should be true')
if (env.SMTP_TLS_REJECT_UNAUTHORIZED === 'false') failures.push('SMTP_TLS_REJECT_UNAUTHORIZED must not be false in production')
if (!env.UPSTASH_REDIS_REST_URL || !env.UPSTASH_REDIS_REST_TOKEN) warnings.push('Upstash Redis is not configured; distributed rate limiting may degrade to local fallback')
...
if (failures.length) {
  console.error(`\nSecurity configuration gate FAILED (${failures.length} blocking issue${failures.length === 1 ? '' : 's'}).`)
  process.exit(1)
}

console.log('\nSecurity configuration gate PASSED.')
